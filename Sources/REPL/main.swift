...
func applyLet(arguments: [Expr], env: Env) throws -> Expr? {
    guard arguments.count == 2 else {
        throw EvalError.wrongNumberOfArgument(inOperation: "let*", expected: 2.description, found: arguments.count.description)
    }
...
//        let result = try EVAL(expr, initialReplEnv())
//        print(result)
//    }
//} catch {
//    print("Error: \(error)")
//}
//
//try print(rep("(* 3 (+ 2 2))", initialReplEnv()).output)
...
//"""
//
//try print(rep(example, initialReplEnv()).output)
...
let myRepl = REPL(env: initialReplEnv())
